 ...
 */
async function modifyRecord(sql) {
  try {
...
async function deleteEmployee(employeeId) {
  return await modifyRecord(`DELETE FROM employee WHERE employee.id = ${employeeId}`, "employee");
}

// Delete Department
async function deleteDepartment(departmentId) {
  return await modifyRecord(`DELETE FROM department WHERE department.id = ${departmentId}`, "epartment");
}

// Delete Role
async function deleteRole(roleId) {
  return await modifyRecord(`DELETE FROM role WHERE role.id = ${roleId}`, "role");
}
