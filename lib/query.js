...
async function viewAllManagers(db) {
  return getAll(db,
...
async function departmentBudget(db, department_id) {
...
async function deleteEmployee(db, employeeId) {
  return await modifyRecord(db, `DELETE FROM employee WHERE employee.id = ${employeeId}`, "employee");
}

// Delete Department
async function deleteDepartment(db, departmentId) {
  return await modifyRecord(db, `DELETE FROM department WHERE department.id = ${departmentId}`, "epartment");
}
// Delete Role
async function deleteRole(db, roleId) {
  return await modifyRecord(db, `DELETE FROM role WHERE role.id = ${roleId}`, "role");
}
