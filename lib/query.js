 ...
 */
async function modifyRecord(db, sql, description) {
  try {
    const result = await db.query(sql);
    return result.affectedRows;
  } catch (err) {
    //throw new Error("Failed to modify " + description);
    console.log("Failed to modify " + description);
    return 0;
  }
}
...
async function deleteDepartment(db, departmentId) {
  return modifyRecord(db, `DELETE FROM department WHERE department.id = ${departmentId}`, "Department");
}
// Delete Role
