...
async function viewDepartmentBudgetAsync() {
  ...
  if (!dept) return;
  const res = await query.department.view.budget(dept.id);
  console.table(res);
}
...
async function viewEmployeesByDeptAsync() {
  const dept = await getDepartmentAsync();
  if (!dept) return;
  return await query.employee.view.byDepartment(dept.name);
}
...
async function viewEmployeesByManagerAsync() {
  ...
  if (!mgr) return;
  return await query.employee.view.byManager(mgr.name);
}
...
async function removeEmployeeAsync() {
  ...
  if (!employee) return;
  const count = await query.employee.delete(employee.id);
...
async function removeRoleAsync() {
  ...
  if (!role) return;
  const count = await query.role.delete(role.id);
...
async function removeDepartmentAsync() {
  ...
  if (!dept) return;
  const count = await query.department.delete(dept.id);
