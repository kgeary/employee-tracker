...
async function subMenuAsync(subMenu) {
  ...
  if (subMenu.viewFunc) {
    ...
    // If an output formatter was provided use that to console log
    if (subMenu.output) {
    ...
    // Should not be encountered unless the subMenu object was configured wrong
...
async function viewDepartmentBudgetAsync(db) {
  ...
  const department = userOption.department;
  const id = helper.getDepartmentId(departments, department);
  const res = await query.view.departmentBudget(db, id);
  console.table(res);
}
