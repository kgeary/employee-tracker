 ...
 * @param {object} queryPath (Example: query.employee, query.role)
 * @param {string} type Type of record to remove, used for prompts
 */
async function removeRoutine(promptFunc, queryPath, type) {
  const data = await promptFunc(`Select ${type} to Remove`);
  if (!data) return;
  const count = await queryPath.delete(data.id);
  helper.checkRemove(count, data.name);
  if (count > 0) {
    const updatedTable = await queryPath.view.all();
...
async function removeRoleAsync() {
  await removeRoutine(getRoleAsync, query.role, "a Role");
}
...
async function updateRoleAsync() {
  ...
  if (count > 0) {
    const updatedTable = await query.employee.view.all();
    console.table(updatedTable);
  }
...
async function addEmployeeAsync() {
  ...
  // Create an Employee and send the request to the database
  try {
    ...
    const updatedEmployees = await query.employee.view.all();
...
async function addRoleAsync() {
  ...
  try {
    ...
    const updatedRoles = await query.role.view.all();
...
async function addDepartmentAsync() {
  ...
  try {
    ...
    const updatedDepartments = await query.department.view.all();
