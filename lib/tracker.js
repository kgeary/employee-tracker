 ...
 * @param {object} queryTable , 
 * @param {string} type Type of record to remove, used for prompts
 */
async function removeRoutine(promptFunc, queryTable, type) {
  const data = await promptFunc(`Select ${type} to Remove`);
  if (!data) return;
  const count = await queryTable.delete(data.id);
  helper.checkRemove(count, data.name);
  if (count > 0) {
    const updatedTable = await queryTable.view.all();
...
async function removeRoleAsync() {
  await removeRoutine(
    getRoleAsync, query.role, "a Role");
}
